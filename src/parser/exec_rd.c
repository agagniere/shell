...
/*   Updated: 2017/03/16 11:01:24 by angagnie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				exec_rdf(t_sh_operator *self, t_sh_context *w)
{
	...
	fd = open(ft_string(sh_resolve(left)), this->flags, S_IRWXU | S_IROTH);
...
int				exec_rd(t_sh_operator *self, t_sh_context *w)
{
	...
	fd = ft_atoi(ft_string(sh_resolve(left)));
	return (_exec_rd(self, fd));
}

t_string		sh_resolve(union u_sh_node *self)
{
	t_string	ans[1];
	int			fd[3];
	int			ret;
